  ...
  if(isset($_POST['delete_booking']))
    {
            ...
            $status  = $_POST['status'];
            $query="update bookings set start_date=?, end_date=?, total_price=?,  created_at=?,  status=? where user_id=?";
            $stmt = $mysqli->prepare($query);
            $rc=$stmt->bind_param('sssssi',  $start_date, $end_date, $total_price, $created_at, $status, $u_id);
            $stmt->execute();
                if($stmt)
                {
