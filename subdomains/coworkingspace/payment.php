<?php
    ...
    // Retrieve the selected plan from the query parameter
    if (isset($_GET['plan'])) {
        $selectedPlan = $_GET['plan'];
    } else {
        // Handle the case if 'plan' parameter is not set
        echo "No plan selected!";
        exit;
    }

    // Set the unit amount based on the selected plan
    if ($selectedPlan == 'Basic') {
        $unitAmount = 4900; // $49 in cents
    } elseif ($selectedPlan == 'Professional') {
        $unitAmount = 9900; // $99 in cents
    } elseif ($selectedPlan == 'Business') {
