<?php
define('DB_SERVER', (getenv('DB_HOST') ?: (file_exists('/.dockerenv') ? 'krazeplanet' : '127.0.0.1')));
define('DB_USER', 'root');
define('DB_PASS', '');
define('DB_NAME', 'hms');

// Initial connection to server to ensure database exists
$init_con = @mysqli_connect(DB_SERVER, DB_USER, DB_PASS);
if ($init_con) {
    @mysqli_query($init_con, "CREATE DATABASE IF NOT EXISTS `" . DB_NAME . "` DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;");
...
// Auto-provision schema and seed data if database is empty
if ($con) {
...
(6, 'Hoyt Drake', 'Dolore aute mollit e', 'Voluptas quas natus ', 'female', 'bofileqe@mailinator.com', 'b916810cd9a5e9689ce64de6223d3da3', '2022-06-13 13:47:50', NULL);",
  ...
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=7;",
        ...
        foreach ($schema_statements as $sql_stmt) {
            if (!empty(trim($sql_stmt))) {
                @mysqli_query($con, $sql_stmt);
            }
        }

        // Ensure default admin user exists
        $chkAdmin = @mysqli_query($con, "SELECT id FROM admin WHERE username='admin'");
        if (!$chkAdmin || mysqli_num_rows($chkAdmin) == 0) {
            @mysqli_query($con, "INSERT INTO admin (username, password) VALUES ('admin', '21232f297a57a5a743894a0e4a801fc3')");
        }
    }
}
