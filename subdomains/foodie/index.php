...
// Fetch Blogs
$blogs = $pdo->query("SELECT * FROM blogs ORDER BY id ASC LIMIT 3")->fetchAll();
?>
<!DOCTYPE html>
<html lang="en">
    ...
    <div class="container">
      ...
      <nav class="navbar" data-navbar>
        <ul class="navbar-list">
          ...
          <li class="nav-item">
            <a href="#reservation-section" class="navbar-link" data-nav-link>Book Table</a></li>
<li class="nav-item"><a href="admin/index.php" class="navbar-link" target="_blank" style="color:var(--deep-saffron);font-weight:700;">Admin</a>
          </li>
      ...
      <div class="header-btn-group">
        ...
        <a href="#reservation-section" class="btn btn-hover" onclick="openReservationModal(); return false;">Reservation</a>
  ...
  <div class="search-container" data-search-container>
    <form action="index.php#food-menu" method="get" class="search-box">
      <input type="search" name="search" value="<?php echo htmlspecialchars($search_query); ?>" aria-label="Search here" placeholder="Type keywords (e.g. Pizza, Burger)..."
        class="search-input">
    ...
    <article>
      ...
      <section class="hero" id="home" style="background-image: url('./assets/images/hero-bg.jpg')">
        <div class="container">
          ...
          <div class="hero-content">
            ...
            <a href="#reservation-section" class="btn" onclick="openReservationModal(); return false;">Book A Table</a>
      ...
      <section class="section food-menu" id="food-menu">
        <div class="container">
          ...
          <p class="section-text">
            Food is any substance consumed to provide nutritional support for an organism. Hand-crafted daily with fresh farm ingredients.
          </p>

          <ul class="fiter-list filter-list" style="display:flex; flex-wrap:wrap; justify-content:center; gap:10px; margin-bottom:40px; list-style:none;">
            <li>
              <a href="index.php?cat=All#food-menu" class="filter-btn <?php echo ($cat_filter === 'All' && empty($search_query)) ? 'active' : ''; ?>" style="display:inline-block; text-decoration:none; cursor:pointer;">All</a>
            </li>
            <li>
              <a href="index.php?cat=Pizza#food-menu" class="filter-btn <?php echo ($cat_filter === 'Pizza') ? 'active' : ''; ?>" style="display:inline-block; text-decoration:none; cursor:pointer;">Pizza</a>
            </li>
            <li>
              <a href="index.php?cat=Burger#food-menu" class="filter-btn <?php echo ($cat_filter === 'Burger') ? 'active' : ''; ?>" style="display:inline-block; text-decoration:none; cursor:pointer;">Burger</a>
            </li>
            <li>
              <a href="index.php?cat=Drinks#food-menu" class="filter-btn <?php echo ($cat_filter === 'Drinks') ? 'active' : ''; ?>" style="display:inline-block; text-decoration:none; cursor:pointer;">Drinks</a>
            </li>
            <li>
              <a href="index.php?cat=Sandwich#food-menu" class="filter-btn <?php echo ($cat_filter === 'Sandwich') ? 'active' : ''; ?>" style="display:inline-block; text-decoration:none; cursor:pointer;">Sandwich</a>
            </li>
          </ul>

          <?php if(!empty($search_query)): ?>
            <div style="text-align:center;margin-bottom:20px;color:var(--eerie-black-1);">
              <span>Showing search results for "<strong><?php echo htmlspecialchars($search_query); ?></strong>" — <a href="index.php#food-menu" style="color:var(--deep-saffron);text-decoration:underline;">Clear Search</a></span>
            </div>
          <?php endif; ?>
          ...
          <ul class="food-menu-list">
            <?php if(empty($food_items)): ?>
              <li style="grid-column: 1 / -1; text-align:center; padding: 40px;">
                <p style="font-size:18px; color:var(--sonic-silver);">No delicious dishes found in this selection. Try another category!</p>
              </li>
            <?php else: ?>
              <?php foreach($food_items as $dish): ?>
                <li>
                  <div class="food-menu-card">
                    <div class="card-banner">
                      <img src="<?php echo htmlspecialchars($dish['image']); ?>" width="300" height="300" loading="lazy"
                        alt="<?php echo htmlspecialchars($dish['title']); ?>" class="w-100">

                      <?php if(!empty($dish['badge'])): ?>
                        <div class="badge"><?php echo htmlspecialchars($dish['badge']); ?></div>
                      <?php endif; ?>

                      <div class="btn-wrapper">
                        <button class="btn-card js-order-btn" title="Order Now"
                          data-id="<?php echo $dish['id']; ?>"
                          data-title="<?php echo htmlspecialchars($dish['title'], ENT_QUOTES); ?>"
                          data-price="<?php echo $dish['price']; ?>"
                          onclick="triggerOrder(this)">
                    ...
                    <div class="wrapper">
                      <p class="category"><?php echo htmlspecialchars($dish['category']); ?></p>
                    ...
                    <h3 class="h3 card-title"><?php echo htmlspecialchars($dish['title']); ?></h3>

                    <div class="price-wrapper">
                      <p class="price-text">Price:</p>
                      <data class="price">$<?php echo number_format($dish['price'], 2); ?></data>

                      <?php if(!empty($dish['discount_price'])): ?>
                        <del class="del">$<?php echo number_format($dish['discount_price'], 2); ?></del>
                      <?php endif; ?>
                    </div>

                    <button class="btn btn-hover js-order-btn" style="margin-top:14px; width:100%; justify-content:center;"
                      data-id="<?php echo $dish['id']; ?>"
                      data-title="<?php echo htmlspecialchars($dish['title'], ENT_QUOTES); ?>"
                      data-price="<?php echo $dish['price']; ?>"
                      onclick="triggerOrder(this)">
      ...
      <section class="section section-divider white testi" id="testimonials">
        <div class="container">
          ...
          <p class="section-text">
            Food is any substance consumed to provide nutritional support for an organism. Here is what our food lovers say!
          </p>

          <ul class="testi-list has-scrollbar" style="display:flex;gap:30px;overflow-x:auto;padding-bottom:15px;">
            <?php foreach($testimonials as $rev): ?>
              <li class="testi-item" style="min-width:320px;flex:1;">
                <div class="testi-card">
                  <div class="profile-wrapper">
                    <figure class="avatar">
                      <img src="<?php echo htmlspecialchars($rev['author_image']); ?>" width="80" height="80" loading="lazy"
                        alt="<?php echo htmlspecialchars($rev['author_name']); ?>">
                    </figure>

                    <div>
                      <h3 class="h4 testi-name"><?php echo htmlspecialchars($rev['author_name']); ?></h3>
                      <p class="testi-title"><?php echo htmlspecialchars($rev['author_role']); ?></p>
                    </div>
                  </div>

                  <blockquote class="testi-text">
                    "<?php echo htmlspecialchars($rev['feedback_text']); ?>"
                  </blockquote>

                  <div class="rating-wrapper">
                    <?php for($s=1; $s<=$rev['rating']; $s++): ?>
                      <ion-icon name="star"></ion-icon>
                    <?php endfor; ?>
                  </div>
                </div>
      ...
      <section class="section section-divider white blog" id="blog">
        <div class="container">
          ...
          <h2 class="h2 section-title">
            This Is What We Call <span class="span">Food Stories</span>
          </h2>

          <p class="section-text">
            Food is any substance consumed to provide nutritional support for an organism. Explore our latest culinary articles.
          </p>
          ...
          <ul class="blog-list">
            <?php foreach($blogs as $blg): ?>
              <li>
                <div class="blog-card">
                  <div class="card-banner">
                    <img src="<?php echo htmlspecialchars($blg['image']); ?>" width="600" height="390" loading="lazy"
                      alt="<?php echo htmlspecialchars($blg['title']); ?>" class="w-100">

                    <div class="badge"><?php echo htmlspecialchars($blg['category']); ?></div>
                  </div>

                  <div class="card-content">
                    <div class="card-meta-wrapper">
                      <a href="#blog" class="card-meta-link">
                        <ion-icon name="calendar-number-outline"></ion-icon>
                        <time class="meta-info" datetime="<?php echo $blg['publish_date']; ?>"><?php echo date('M d, Y', strtotime($blg['publish_date'])); ?></time>
                      </a>

                      <a href="#blog" class="card-meta-link">
                        <ion-icon name="person-outline"></ion-icon>
                        <p class="meta-info"><?php echo htmlspecialchars($blg['author_name']); ?></p>
                      </a>
                    </div>

                    <h3 class="h3">
                      <a href="#blog" class="card-title"><?php echo htmlspecialchars($blg['title']); ?></a>
                    </h3>

                    <p class="blog-text">
                      <?php echo htmlspecialchars($blg['excerpt']); ?>
                    </p>
  ...
  <footer class="footer">
    ...
    <div class="footer-top" style="background-image: url('./assets/images/footer-illustration.png')">
      <div class="container">
        ...
        <form id="footerBookingForm" onsubmit="handleFooterReservation(event)" class="footer-form" id="reservation-section">
          ...
          <div class="input-wrapper">
            ...
            <input type="date" name="booking_date" value="<?php echo date('Y-m-d'); ?>" required aria-label="Reservation date" class="input-field">
          </div>

          <textarea name="message" placeholder="Special Requests / Table preference (Optional)" aria-label="Message" class="input-field"></textarea>
  ...
  <!-- ══════════════════════════════════════════════════════════════════════ -->
  <div id="reservationModal" class="foodie-modal-backdrop">
    <div class="foodie-modal-card">
      <div class="foodie-modal-header">
        <h3 class="foodie-modal-title">🍽️ Book A Table</h3>
        <button type="button" class="foodie-modal-close" onclick="closeReservationModal()">&times;</button>
      </div>
      <form id="reservationForm" onsubmit="handleModalReservation(event)">
        <div class="foodie-form-row">
          <div class="foodie-form-group">
            <label>Full Name *</label>
            <input type="text" name="full_name" required placeholder="e.g. John Doe" class="foodie-input">
          </div>
          <div class="foodie-form-group">
            <label>Phone Number *</label>
            <input type="tel" name="phone" required placeholder="e.g. +1 555-019-2831" class="foodie-input">
          </div>
        </div>
        ...
        <div class="foodie-form-row">
          <div class="foodie-form-group">
            ...
            <input type="email" name="email_address" placeholder="john@example.com" class="foodie-input">
          </div>
          <div class="foodie-form-group">
            <label>Number of Guests *</label>
            <select name="total_person" class="foodie-input">
        ...
        <div class="foodie-form-row">
          <div class="foodie-form-group">
            <label>Date *</label>
            <input type="date" name="booking_date" value="<?php echo date('Y-m-d'); ?>" required class="foodie-input">
          </div>
          <div class="foodie-form-group">
            <label>Time *</label>
            <select name="reservation_time" class="foodie-input">
        ...
        <div class="foodie-form-group" style="margin-bottom:20px;">
          <label>Special Requests / Seating Preference</label>
          <textarea name="message" rows="2" placeholder="Window view, anniversary setup, dietary notes..." class="foodie-input"></textarea>
        </div>

        <button type="submit" class="btn btn-hover" style="width:100%;justify-content:center;padding:12px;font-size:16px;">
  ...
  <!-- MODAL: ORDER NOW -->
  <!-- ══════════════════════════════════════════════════════════════════════ -->
  <div id="orderModal" class="foodie-modal-backdrop">
    <div class="foodie-modal-card">
      <div class="foodie-modal-header">
        <h3 class="foodie-modal-title">🛵 Place Food Delivery</h3>
        <button type="button" class="foodie-modal-close" onclick="closeOrderModal()">&times;</button>
      </div>
      <form id="orderForm" onsubmit="handleOrderSubmit(event)">
        <input type="hidden" name="item_id" id="orderItemId">
        
        <div style="background:rgba(248,69,37,0.08);padding:14px;border-radius:8px;margin-bottom:18px;display:flex;justify-content:space-between;align-items:center;">
          <div>
            <h4 id="orderItemTitle" style="font-size:16px;font-weight:700;color:var(--eerie-black-1);">Dish Title</h4>
            <span id="orderItemPrice" style="color:var(--deep-saffron);font-weight:700;">$0.00</span>
          </div>
          <div style="display:flex;align-items:center;gap:10px;">
            <label style="margin:0;font-size:13px;font-weight:600;">Qty:</label>
            <input type="number" name="quantity" id="orderQuantity" min="1" max="20" value="1" onchange="updateOrderTotal()" class="foodie-input" style="width:70px;text-align:center;">
          </div>
        </div>
        ...
        <div class="foodie-form-row">
          <div class="foodie-form-group">
            ...
            <input type="text" name="customer_name" required placeholder="Full Name" class="foodie-input">
          </div>
          <div class="foodie-form-group">
            <label>Contact Phone *</label>
            <input type="tel" name="customer_phone" required placeholder="+1 555-019-2831" class="foodie-input">
          </div>
        </div>

        <div class="foodie-form-group">
          <label>Email Address</label>
          <input type="email" name="customer_email" placeholder="email@example.com" class="foodie-input">
        </div>

        <div class="foodie-form-group">
          <label>Delivery Address *</label>
          <textarea name="delivery_address" rows="2" required placeholder="Street address, building, floor/flat number..." class="foodie-input"></textarea>
        </div>

        <div class="foodie-form-group" style="margin-bottom:20px;">
          <label>Special Instructions / Notes</label>
          <input type="text" name="order_notes" placeholder="Extra napkins, contactless drop-off..." class="foodie-input">
        </div>

        <button type="submit" class="btn btn-hover" style="width:100%;justify-content:center;padding:12px;font-size:16px;">
          Submit Delivery Order (<span id="orderTotalDisplay">$0.00</span>)
        </button>
      </form>
    </div>
  </div>
  ...
  <div id="foodieToastContainer" class="foodie-toast-container"></div>

  <!-- MODAL & INTERACTIVE STYLES -->
  <style>
    .foodie-modal-backdrop {
      position: fixed;
    ...
    .foodie-input:focus {
      ...
      box-shadow: 0 0 0 3px rgba(248,69,37,0.15);
    }
  ...
  <script>
    ...
    function showFoodieToast(message, type = 'success') {
      const container = document.getElementById('foodieToastContainer');
      if (!container) return;
      const toast = document.createElement('div');
      toast.className = `foodie-toast foodie-toast-${type}`;
      toast.innerHTML = `<span style="flex:1;">${message}</span><span style="cursor:pointer;font-size:18px;" onclick="this.parentElement.remove()">&times;</span>`;
    ...
    async function handleModalReservation(e) {
      e.preventDefault();
      const form = e.target;
      ...
      try {
        ...
        if (data.status === 'success') {
          showFoodieToast(data.message, 'success');
          form.reset();
          closeReservationModal();
        } else {
          showFoodieToast(data.message, 'error');
        }
      } catch (err) {
        showFoodieToast('Failed to submit reservation. Please try again.', 'error');
      }
    }

    // Embedded Footer Reservation Form
    async function handleFooterReservation(e) {
      e.preventDefault();
      const form = e.target;
      const formData = new FormData(form);
      ...
      try {
        ...
        showFoodieToast('Failed to submit reservation.', 'error');
      }
    }

    // Trigger Order from Button Data Attributes
    function triggerOrder(btn) {
      const id = btn.getAttribute('data-id');
      const title = btn.getAttribute('data-title');
      const price = parseFloat(btn.getAttribute('data-price')) || 0;
      openOrderModal(id, title, price);
    }

    function openOrderModal(id, title, price) {
      document.getElementById('orderItemId').value = id;
      document.getElementById('orderItemTitle').textContent = title;
      currentItemUnitPrice = parseFloat(price);
      document.getElementById('orderItemPrice').textContent = `$${currentItemUnitPrice.toFixed(2)}`;
    ...
    function updateOrderTotal() {
      ...
      const total = (currentItemUnitPrice * qty).toFixed(2);
    ...
    async function handleOrderSubmit(e) {
      ...
      try {
        ...
        showFoodieToast('Failed to place order. Please try again.', 'error');
      }
    }
  </script>
