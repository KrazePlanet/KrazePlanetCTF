...
// ── Table ─────────────────────────────────────────────────────────────────────
$db->query("CREATE TABLE IF NOT EXISTS pixeleet_users (
...
if ($check && $check->fetch_assoc()['c'] == 0) {
    ...
    $st = $db->prepare("INSERT INTO pixeleet_users (id, name, email, password, bio) VALUES (?, ?, ?, ?, ?)");
    ...
    $st = $db->prepare("INSERT INTO pixeleet_users (name, email, password, bio) VALUES (?, ?, ?, ?)");
...
if ($isRegister && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$name || !$email || !$pass) {
        ...
        if ($st->execute()) {
            ...
            $st->close();
            session_regenerate_id(true);
            $_SESSION['pixeleet_uid'] = $uid;
            $_SESSION['pixeleet_name'] = $name;
            header('Location: index.php');
            exit;
        } else {
...
// ── Handle File Upload ────────────────────────────────────────────────────────
if ($isUpload && $_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_SESSION['pixeleet_uid'])) {
    $uid = (int)$_SESSION['pixeleet_uid'];
    if (!isset($_FILES['avatar']) || $_FILES['avatar']['error'] !== UPLOAD_ERR_OK) {
        ...
        if ($file['size'] > 5 * 1024 * 1024) {
            ...
            if (move_uploaded_file($file['tmp_name'], $dest)) {
                ...
                $st = $db->prepare("UPDATE pixeleet_users SET avatar_path = ?, avatar_alt = ? WHERE id = ?");
...
if (!$isRegister && !$isUpload && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($user && password_verify($pass, $user['password'])) {
        session_regenerate_id(true);
        $_SESSION['pixeleet_uid'] = (int)$user['id'];
        $_SESSION['pixeleet_name'] = $user['name'];
        header('Location: index.php');
        exit;
    } else {
...
$currentUser = null;
if (!empty($_SESSION['pixeleet_uid'])) {
    $uid = (int)$_SESSION['pixeleet_uid'];
...
$allUsers = $db->query("SELECT id, name, bio FROM pixeleet_users ORDER BY id ASC");
...
<div class="header">
    ...
    <div class="header-right">
        ...
        <?php else: ?>
            ...
            <a href="index.php?action=register" class="btn-auth"><i class="fas fa-user-plus"></i> Join</a>
        <?php endif; ?>
    </div>
</div>
    ...
    <?php if (!$isRegister): ?>
        ...
        <div class="card" style="max-width:420px;margin:0 auto;">
            ...
            <div class="auth-toggle">
                Don't have an account? <a href="index.php?action=register">Join Pixeleet</a>
            </div>
        </div>
...
<?php elseif ($searchQuery === ''): ?>
    ...
    <?php if ($viewUser): ?>
        ...
        <!-- ═══════════════════════════════════════════════════════════════ -->
        <a href="index.php" style="font-size:14px;color:#6366f1;margin-bottom:16px;display:inline-block;">
        ...
        <div class="card">
            ...
            <?php if ($allUsers && $allUsers->num_rows > 0): ?>
                <?php while ($u = $allUsers->fetch_assoc()): ?>
                    ...
                    <a href="index.php?action=view&id=<?= (int)$u['id'] ?>" class="user-card">
                        <?php
                        $uInitials = '';
